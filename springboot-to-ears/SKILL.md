...
- List **every** domain-specific property of the entity. Embedded JSON-column structured objects, child entities, **and entities reached through a `Reference to <Entity>` / `List of <Entity>` property** each get their **own** `###` sub-block (or a clearly-labelled nested table) — do not flatten or omit them. Follow every such reference and catalogue the target entity in full; recurse transitively with no depth limit; catalogue each distinct entity once (deduplicate repeats).
...
- Shared base-entity audit fields (created/updated by + at, soft-delete flag, optimistic-locking version) are **not** repeated per entity. State them once in a note at the top of the section that points to Cross-Cutting Requirements → Audit and Record Lifecycle (Step 0b).
...
- Do not re-state shared base entity fields per entity — reference the Ubiquitous requirements from Step 0b once, via the note at the top of the Domain Entities and Properties section, rather than repeating audit/soft-delete/version rows in every entity table.
...
> Shared audit, soft-delete, and optimistic-locking fields apply to every entity below and are described once under Cross-Cutting Requirements → Audit and Record Lifecycle; they are not repeated per entity. Every entity referenced by another (via a `Reference to <Entity>` / `List of <Entity>` property) is catalogued below with its own full property sub-block, recursed transitively and deduplicated (Rule 13). The `Property` column uses the exact codebase field name; plain English goes in `Meaning`.
...
- [ ] **Rule 13:** Every entity catalog in Domain Entities and Properties uses business-language property names and business-level types — no Java field names, column names, or class names — in a four-column `Property | Type | Meaning | Constraints` table.
...
- [ ] Domain Entities and Properties contains a `###` sub-block with a four-column `Property | Type | Meaning | Constraints` table for **every** entity in the traced call chain (including embedded JSON objects and child entities); base-entity audit fields are referenced once via the header note, not repeated per entity.
...
24. **Domain Entities and Properties catalogs every entity in the call chain with all its properties (Rule 13).** Reading an entity in Step 2c is not done until its properties are written to that section as a `Property | Type | Meaning | Constraints` table. Embedded JSON objects and child entities get their own sub-blocks; shared audit/soft-delete/version fields are referenced once, not repeated.